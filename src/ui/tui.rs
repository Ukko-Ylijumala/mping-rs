...
static CON_NFO_T: Constraint = Constraint::Length(12);
...
impl TerminalGuard {
    /**
    Set up the TUI. `on_panic` runs from the panic hook before the terminal
    is restored; the app passes its quit path, since after the hook has torn
    down the terminal the app must not keep drawing (a panic in a ping task
    or the keyboard thread would otherwise leave it running on a dead TUI).
    */
    pub fn new<F>(interval: Duration, logger: Arc<MessageBuffer>, on_panic: F) -> Result<Self>
    where
